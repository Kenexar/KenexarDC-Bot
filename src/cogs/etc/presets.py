...
class Preset:
    ...
    def whitelist(mode=str, member=int, rank=int):
        ...
        if mode == 'list':
            cur_db.execute(f"SELECT uid, rank FROM whitelist WHERE name='{PROJECT_NAME}'")
            fetcher = cur_db.fetchall()

            embed = nextcord.Embed(title='Whitelist',
                                   color=EMBED_ST)

            if fetcher:
                for i in fetcher:
                    embed.add_field(name=i[0], value=f'Rank: {WHITELIST_RANKS[i[1]]}', inline=False)
            else:
                return 'Cannot find any entries'
            return embed

        elif mode == 'add':
            cur_db.execute("INSERT INTO whitelist(name, uid, rank) VALUES (%s, %s, %s)", (PROJECT_NAME, member, rank))
            dbBase.commit()
            return f'Added <@{member}> to the [BOT]whitelist'

        elif mode == 'remove':
            cur_db.execute("DELETE FROM whitelist WHERE uid=%s and name=%s;", (member, PROJECT_NAME))
            dbBase.commit()
